		...
		if (!world.isRemote) {
			...
			if (damage == DATA.bioVelocity.ordinal() || damage == DATA.supernaturalvelocity.ordinal()) {
				...
				if (currentSpeed == null || damage == DATA.bioVelocity.ordinal() && currentSpeed.getAmplifier() < 1 || damage == DATA.supernaturalvelocity.ordinal() && currentSpeed.getAmplifier() < 3) {
					...
					PacketPipeline.sendPacketToAll(PacketTypeHandler.populatePacket(new PacketSound(PacketSound.SOUND_VELOCITY_USE, player.posX, player.posY, player.posZ, 1.2F, 1F)));
				} else
					return is;
			}
			...
			if (damage == DATA.camoPowder.ordinal()) {
				...
				if (currentVisibility == null || damage == DATA.camoPowder.ordinal() && currentVisibility.getAmplifier() < 3) {
					...
					PacketPipeline.sendPacketToAll(PacketTypeHandler.populatePacket(new PacketSound(PacketSound.SOUND_CAMO_USE, player.posX, player.posY, player.posZ, 1.2F, 1F)));
				} else
					return is;
			} else
				return is;
