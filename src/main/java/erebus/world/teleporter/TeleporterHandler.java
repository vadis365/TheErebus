	...
	private void transferEntity(Entity entity, int dimensionId)
	{
		...
		if (!world.isRemote && !entity.isDead)
		{
			if (entity instanceof EntityPlayerMP)
			{
				...
				waitingPlayers.put(player.getGameProfile().getId(), (byte) 40); // if
				// there
				// are
				// any
				// issues,
			...
			{ // TODO we cannot handle this, would result in container breaking
				// in both worlds and duplicate items; find some sneaky solution
				// around this issue
				// fixme copy paste
