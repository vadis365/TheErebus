...
public class BiomeDecoratorSubterraneanSavannah extends BiomeDecoratorBaseErebus{
	...
	protected void modifyOreGen(OreSettings oreGen, OreType oreType, boolean extraOres){
		switch(oreType){
			case GOLD:
				oreGen.setChance(0.75F).setIterations(extraOres ? 1 : 2).setY(50,126).generate(world,rand,x,z); // split
				// into
				// 3
				// parts,
				// higher
				...
				// less
				// gold
				oreGen.setChance(0.9F).setIterations(extraOres ? 1 : 2,extraOres ? 2 : 3).setY(25,50).generate(world,rand,x,z); // balanced
				// so
				// there
				// are
				// more
			...
			case DIAMOND:
				...
				break; // 2 diamonds per cluster, ~7-8 times smaller area thus
			// lowered chance and iterations
			case JADE:
				oreGen.setChance(0.3F).setIterations(0,3);
				break; // less common
			case PETRIFIED_WOOD:
				oreGen.setChance(0.5F).setIterations(0,extraOres ? 2 : 3).setY(5,64);
				break; // less common and lowered area ~2 times, thus lowered
