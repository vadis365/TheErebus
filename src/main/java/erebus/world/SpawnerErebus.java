...
public final class SpawnerErebus{
	...
	private void runGradualSpawning(WorldServer world){
		...
		if (totalAmount >= Math.min(spawnChunks.size()>>1,MAX_MOBS_PER_WORLD)/(world.difficultySetting == EnumDifficulty.PEACEFUL ? 2 : 1))return;
		...
		for(ChunkCoordIntPair coords:chunksToTest){
			...
			while(attempts < 4 && spawned < 2){
				...
				if (rand.nextFloat() > biome.getSpawningChance() || (entry.worldLimit != -1 && entityCount.get(entry.mobClass) >= entry.worldLimit))continue;
