...
public class WorldGenPonds extends WorldGenerator{
	...
	public boolean generate(World world, Random rand, int x, int y, int z){
		...
		for(int xx = 0; xx < 16; ++xx)
			for(int zz = 0; zz < 16; ++zz)
				for(int yy = 0; yy < 8; ++yy){
					...
					if (yy >= 4 && mat.isLiquid() || yy < 4 && !mat.isSolid() && world.getBlock(x+xx,y+yy,z+zz)!=Blocks.flowing_water)return false;
				}

		for(int xx = 0; xx < 16; ++xx)
			for(int zz = 0; zz < 16; ++zz)
				for(int yy = 0; yy < 8; ++yy)
					if (placeWater[(xx*16+zz)*8+yy])world.setBlock(x+xx,y+yy,z+zz,yy >= 4?Blocks.air:Blocks.flowing_water,0,3);
			...
			for(int zz = 0; zz < 16; ++zz)
				for(int yy = 4; yy < 8; ++yy)
					if (placeWater[(xx*16+zz)*8+yy] && world.getBlock(x+xx,y+yy-1,z+zz) == Blocks.dirt && world.getSavedLightValue(EnumSkyBlock.Sky,x+xx,y+yy,z+zz)>0){
						...
						if (biome.topBlock == Blocks.mycelium)world.setBlock(x+xx,y+yy-1,z+zz,Blocks.mycelium,0,2);
						else world.setBlock(x+xx,y+yy-1,z+zz,Blocks.grass,0,2);
					}

		for(int xx = 0; xx < 16; ++xx)
			for(int zz = 0; zz < 16; ++zz)
				if (world.isBlockFreezable(x+xx,y+4,z+zz))world.setBlock(x+xx,y+4,z+zz,Blocks.ice,0,2);
		...
		for(int sugarCaneAttempt = 0, xx, yy, zz; sugarCaneAttempt < 60; sugarCaneAttempt++){
			...
			if ((block == Blocks.grass || block == Blocks.sand) && Blocks.reeds.canPlaceBlockAt(world,xx,yy,zz))
				for(int height = 0; height < 1+rand.nextInt(7); height++)
					if (world.isAirBlock(xx,yy+height,zz))world.setBlock(xx,yy+height,zz,Blocks.reeds);
					else break;
		}
		...
		for(int bullRushAttempt = 0, xx, yy, zz; bullRushAttempt < 150; bullRushAttempt++){
			...
			if (block == Blocks.sand && ModBlocks.doubleHeightPlant.canPlaceBlockAt(world,xx,yy,zz))
				for(int height = 0; height < 1; height++)
					if (world.isAirBlock(xx,yy+height,zz)) {
						world.setBlock(xx,yy,zz,ModBlocks.doubleHeightPlant,BlockDoubleHeightPlant.dataBullrushBottom,2);
						world.setBlock(xx,yy+1,zz,ModBlocks.doubleHeightPlant,BlockDoubleHeightPlant.dataBullrushTop,2);
					}
					else break;
		}
