...
public class WorldGenGiantMushrooms extends WorldGenErebus{
	...
	public enum MushroomType{
		BULB_CAPPED(ModBlocks.mushroomCap0), GRANDMAS_SHOES(ModBlocks.mushroomCap1), SARCASTIC_CZECH(ModBlocks.mushroomCap2), KAIZERS_FINGERS(ModBlocks.mushroomCap3), DUTCH_CAP(ModBlocks.mushroomCap4);
	...
	private int getBulbMetadata(int centerX, int centerZ, ChunkCoordinates bulb){
		...
		if (sides == 0 && corners == 2){
			for(int dir = 0, meta; dir < 4; dir++){
				...
				if (meta != 0){
					return meta; // use meta of cap above and to the side -
					// either 2 side cap or full cap
				}
			}
		}
