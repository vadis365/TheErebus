			...
			if (world.isAirBlock(x,y,z) && world.getBlock(x,y-1,z) == Blocks.sand){
				for(int xx = x-4; xx <= x+4; xx++)
					for(int zz = z-4; zz <= z+4; zz++)
						if (!world.isAirBlock(x,y,z)||world.getBlock(xx,y-1,zz)!=Blocks.sand)return false;
