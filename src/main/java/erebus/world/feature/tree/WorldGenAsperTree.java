...
public class WorldGenAsperTree extends WorldGenerator{
	...
	public boolean generate(World world, Random random, int x, int y, int z){
		...
		for(int yy = 0; yy < height; yy++){
			world.setBlock(x,y+yy,z,ModBlocks.logErebusGroup2,BlockLogErebus.dataAsper,3);
			...
			for(int extraWood = 0,extraWoodAttempt = 0; extraWoodAttempt < 5 && extraWood < 3; extraWoodAttempt++){
				...
				world.setBlock(x+offsetX[dir],y+yy,z+offsetZ[dir],ModBlocks.logErebusGroup2,BlockLogErebus.dataAsper+(dir < 2?4:8),3);
				if (yy > 0 && random.nextBoolean()){
					world.setBlock(x+offsetX[dir]*2,y+yy,z+offsetZ[dir]*2,ModBlocks.leavesErebus,BlockLeavesErebus.dataAsperDecay,3);
				}
				++extraWood;
			}
		}
			...
			for(int xx=-1; xx <= 1; xx++){
				for(int zz=-1; zz <= 1; zz++){
					...
					if ((dist <= 1.5D || random.nextDouble()>dist-1.5D) && world.isAirBlock(x+xx,y+yy,z+zz)){
						world.setBlock(x+xx,y+yy,z+zz,ModBlocks.leavesErebus,BlockLeavesErebus.dataAsperDecay,3);
					}
				}
			}
		}

		for(int a = 0; a < 4; a++)world.setBlock(x+offsetX[a],y+height,z+offsetZ[a],ModBlocks.leavesErebus,BlockLeavesErebus.dataAsperDecay,3);
		world.setBlock(x,y+height,z,ModBlocks.leavesErebus,BlockLeavesErebus.dataAsperDecay,3);
