...
public class WorldGenCypressTree extends WorldGenTreeBase {
	...
	private boolean generateSmallTree(World world, Random rand, int x, int y, int z) {
		...
		for (int yy = y + trunkH + 1; yy <= y + trunkH + 2; yy++)
			world.setBlockState(new BlockPos(x, yy, z), leaves, 2);

		for (int a = 0; a < 4; a++) {
			for (int yy = y + nakedTrunkH; yy <= y + trunkH; yy++)
				world.setBlockState(new BlockPos(x + offsetX[a], yy, z + offsetZ[a]), leaves, 2);

			if (leafH - 4 > 1)
				for (int yy = y + nakedTrunkH + 2; yy <= y + trunkH - 2; yy++) {
					if ((yy == y + nakedTrunkH + 2 || yy == y + trunkH - 2) && leafH - 4 > 3 && rand.nextInt(11) == 0)
						continue;
					world.setBlockState(new BlockPos(x + offsetX[a] * 2, yy, z + offsetZ[a] * 2), leaves, 2);
				}
		}
			...
			for (int b = 0; b < 2; b++)
				for (int yy = y + nakedTrunkH + 1; yy <= y + trunkH - 1; yy++) {
					if ((yy == y + nakedTrunkH + 1 || yy == y + trunkH - 1) && leafH - 2 > 3 && rand.nextInt(8) == 0)
						continue;
					world.setBlockState(new BlockPos(x - 1 + 2 * a, yy, z - 1 + 2 * b), leaves, 2);
				}
			...
			if (leafH - 7 > 1)
				for (int yy = y + nakedTrunkH + 3; yy <= y + trunkH - 4; yy++) {
					if ((yy == y + nakedTrunkH + 3 || yy == y + trunkH - 4) && leafH - 7 > 3 && rand.nextInt(10) == 0)
						continue;
					world.setBlockState(new BlockPos(x + offsetX[a] * 3, yy, z + offsetZ[a] * 3), leaves, 2);
				}
		}
			...
			for (int b = 0; b < 2; b++) {
				...
				for (int yy = y + nakedTrunkH + 2; yy <= y + trunkH - 3; yy++) {
					...
					if (!canSkip || rand.nextInt(10) != 0)
						world.setBlockState(new BlockPos(x - 2 + 4 * a, yy, z + -1 + 2 * b), leaves, 2);
					if (!canSkip || rand.nextInt(10) != 0)
						world.setBlockState(new BlockPos(x - 1 + 2 * a, yy, z + -2 + 4 * b), leaves, 2);
				}
			}
		...
		for (int yy = y; yy <= y + trunkH; yy++) {
			...
			if (yy <= y + trunkH - 2)
				for (int a = 0; a < 4; a++) {
					...
					world.setBlockState(new BlockPos(x + offsetX[a], yy, z + offsetZ[a]), genLog ? log.getStateFromMeta(0) : leaves, 2);
				}
		}
			...
			for (int b = 0; b < 2; b++) {
				...
				for (int yy = y + nakedTrunkH + 3; yy <= y + trunkH - 3; yy++) {
					if (yy == y + trunkH - 3 && rand.nextInt(7) == 0)
						continue;
					world.setBlockState(new BlockPos(x - 2 + 4 * a, yy, z - 2 + 4 * b), leaves, 2);
				}
				...
				if (leafH - 9 > 1)
					for (int yy = y + nakedTrunkH + 4; yy <= y + trunkH - 5; yy++) {
						...
						if (!canSkip || rand.nextInt(12) != 0)
							world.setBlockState(new BlockPos(x - 3 + 6 * a, yy, z - 1 + 2 * b), leaves, 2);
						if (!canSkip || rand.nextInt(12) != 0)
							world.setBlockState(new BlockPos(x - 1 + 2 * a, yy, z - 3 + 6 * b), leaves, 2);
					}
			}
