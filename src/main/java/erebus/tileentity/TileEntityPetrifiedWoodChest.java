...
public class TileEntityPetrifiedWoodChest extends TileEntityChest {
    ...
    public TileEntityPetrifiedWoodChest(BlockPetrifiedChest.Type typeIn) {
	...
	public static void registerFixesChest(DataFixer fixer) {
		fixer.registerWalker(FixTypes.BLOCK_ENTITY,
				new ItemStackDataLists(TileEntityPetrifiedWoodChest.class, new String[] { "Items" }));
	}
	...
	private void setNeighbor(TileEntityPetrifiedWoodChest chestTe, EnumFacing side) {
	...
	@Nullable
	protected TileEntityPetrifiedWoodChest getAdjacentChest(EnumFacing side) {
		...
		if (this.isChestAt(blockpos)) {
			...
			if (tileentity instanceof TileEntityPetrifiedWoodChest) {
				TileEntityPetrifiedWoodChest tileentitychest = (TileEntityPetrifiedWoodChest) tileentity;
	...
	private boolean isChestAt(BlockPos posIn) {
		if (this.world == null) {
			...
			return block instanceof BlockPetrifiedChest && ((BlockPetrifiedChest) block).chestType == this.getChestType();
		}
	}
	...
	public void update() {
		...
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + i + j + k) % 200 == 0) {
			...
			for (EntityPlayer entityplayer : this.world.getEntitiesWithinAABB(EntityPlayer.class,
					new AxisAlignedBB((double) ((float) i - 5.0F), (double) ((float) j - 5.0F), (double) ((float) k - 5.0F), (double) ((float) (i + 1) + 5.0F), (double) ((float) (j + 1) + 5.0F), (double) ((float) (k + 1) + 5.0F)))) {
	...
	public void openInventory(EntityPlayer player) {
		if (!player.isSpectator()) {
			...
			if (this.getChestType() == BlockPetrifiedChest.Type.TRAP) {
	...
	public void closeInventory(EntityPlayer player) {
		if (!player.isSpectator() && this.getBlockType() instanceof BlockPetrifiedChest) {
			--this.numPlayersUsing;
			this.world.addBlockEvent(this.pos, this.getBlockType(), 1, this.numPlayersUsing);
			this.world.notifyNeighborsOfStateChange(this.pos, this.getBlockType(), false);
	...
	public BlockPetrifiedChest.Type getChestType() {
		if (this.cachedChestType == null) {
			if (this.world == null || !(this.getBlockType() instanceof BlockPetrifiedChest)) {
				return BlockPetrifiedChest.Type.BASIC;
			}

			this.cachedChestType = ((BlockPetrifiedChest) this.getBlockType()).chestType;
		}
	...
	public Container createContainer(InventoryPlayer playerInventory, EntityPlayer playerIn) {
		...
		return new ContainerPetrifiedWoodChest(playerInventory, this, playerIn);
	}
