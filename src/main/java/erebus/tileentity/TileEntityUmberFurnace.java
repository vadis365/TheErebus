...
public class TileEntityUmberFurnace extends TileEntityBasicInventory implements IFluidHandler {
	...
	public void updateEntity() {
		...
		// Draining buckets
		getInventory()[BUCKET_SLOT] = fillTankWithBucket(getInventory()[BUCKET_SLOT]);
		...
		if (!worldObj.isRemote) {
			if (furnaceBurnTime == 0 && canSmelt()) {
				currentItemBurnTime = furnaceBurnTime = TileEntityFurnace.getItemBurnTime(getInventory()[FUEL_SLOT]);
				if (furnaceBurnTime > 0) {
					flag1 = true;
					if (getInventory()[FUEL_SLOT] != null) {
						getInventory()[FUEL_SLOT].stackSize--;
						if (getInventory()[FUEL_SLOT].stackSize == 0)
							getInventory()[FUEL_SLOT] = getInventory()[FUEL_SLOT].getItem().getContainerItem(getInventory()[FUEL_SLOT]);
					}
				}
			}
	...
	private boolean canSmelt() {
		if (getInventory()[SMELT_SLOT] == null)
			return false;
		else {
			ItemStack is = FurnaceRecipes.smelting().getSmeltingResult(getInventory()[SMELT_SLOT]);
			if (is == null)
				return false;
			if (getInventory()[RESULT_SLOT] == null)
				return true;
			if (!getInventory()[RESULT_SLOT].isItemEqual(is))
				return false;
			int result = getInventory()[RESULT_SLOT].stackSize + is.stackSize;
	...
	private void smeltItem() {
		if (canSmelt()) {
			ItemStack stack = FurnaceRecipes.smelting().getSmeltingResult(getInventory()[SMELT_SLOT]);

			if (getInventory()[RESULT_SLOT] == null)
				getInventory()[RESULT_SLOT] = stack.copy();
			else if (getInventory()[RESULT_SLOT].isItemEqual(stack))
				getInventory()[RESULT_SLOT].stackSize += stack.stackSize;

			tank.drain(FluidContainerRegistry.BUCKET_VOLUME / 10, true);
			getInventory()[SMELT_SLOT].stackSize--;

			if (getInventory()[SMELT_SLOT].stackSize <= 0)
				getInventory()[SMELT_SLOT] = null;
		}
	}
