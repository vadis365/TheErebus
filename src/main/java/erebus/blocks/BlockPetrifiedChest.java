...
public class BlockPetrifiedChest extends BlockContainer {
	...
	public static IInventory getInventory(World world, BlockPos pos) {
		...
		if (object == null)
			return null;
		else if (world.isSideSolid(pos.up(), EnumFacing.DOWN))
			return null;
		else if (isOcelotBlockingChest(world, pos))
			return null;
	...
	public static boolean isOcelotBlockingChest(World world, BlockPos pos) {
		Iterator<EntityOcelot> iterator = world.getEntitiesWithinAABB(EntityOcelot.class, new AxisAlignedBB(pos.up(), pos.add(1, 2, 1)).iterator();
