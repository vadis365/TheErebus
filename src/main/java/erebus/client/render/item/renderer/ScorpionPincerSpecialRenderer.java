...
public class ScorpionPincerSpecialRenderer implements NoDataSpecialModelRenderer {

    private final ScorpionPincerModel model;

    public ScorpionPincerSpecialRenderer(ScorpionPincerModel model) {
        this.model = model;
    }
    ...
    public void submit(@NonNull ItemDisplayContext context, PoseStack pose, SubmitNodeCollector submit, int lightCoords, int overlayCoords, boolean hasFoil, int outlineColor) {
        ...
        submit.submitModelPart(
                model.root(),
                pose,
                model.renderType(Erebus.prefix("textures/special/items/scorpion_pincer.png")),
                lightCoords,
                overlayCoords,
                null,
                false,
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked {
        ...
        public @Nullable SpecialModelRenderer<?> bake(BakingContext context) {
            return new ScorpionPincerSpecialRenderer(new ScorpionPincerModel(context.entityModelSet().bakeLayer(ModItemRendering.SCORPION_PINCER)));
        }
