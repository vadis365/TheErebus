...
public class ModelPuffShroom extends ModelBase {
    ...
    public void render(TileEntityPuffShroom tile) { 
        block.render(0.0625F);
        GL11.glPushMatrix();
        if(tile.animationTicks <= 8)
        	GL11.glTranslatef(0F, 0F - 1F/8 * tile.animationTicks *0.5F, 0F);
        if(tile.animationTicks > 8)
        	GL11.glTranslatef(0F, 0F - 0.5F, 0F);
        if(tile.animationTicks > 1) {
        	if(tile.animationTicks > 8 && tile.animationTicks < 12)
        		GL11.glScalef(1 + 1F/8 * tile.animationTicks *0.25F, 1F, 1 + 1F/8 * tile.animationTicks *0.25F);
        	if(tile.animationTicks >= 12)
        		GL11.glScalef(1 - 1F/8 * tile.animationTicks *0.25F, 1F, 1 - 1F/8 * tile.animationTicks *0.25F);
        	base.render(0.0625F);
        }
        GL11.glPopMatrix();
