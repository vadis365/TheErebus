...
public class ConfigHandler {
	...
	private void syncConfigs() {
		...
		// Biomes & misc
		ModBiomes.undergroundJungleID = config.get("Biomes", "Biome ID of Underground Jungle", 151).getInt(151);
		ModBiomes.volcanicDesertID = config.get("Biomes", "Biome ID of Volcanic Desert", 152).getInt(152);
		ModBiomes.subterraneanSavannahID = config.get("Biomes", "Biome ID of Subterranean Savannah", 153).getInt(153);
		ModBiomes.elysianFieldsID = config.get("Biomes", "Biome ID of Elysian Fields", 154).getInt(154);
		ModBiomes.ulteriorOutbackID = config.get("Biomes", "Biome ID of Ulterior Outback", 155).getInt(155);
		...
		ModBiomes.fieldsSubForestID = config.get("Biomes", "Biome ID of Fields - Forest", 167).getInt(167);
