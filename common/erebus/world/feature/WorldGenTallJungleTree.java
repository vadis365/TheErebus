...
public class WorldGenTallJungleTree extends WorldGenerator{
	...
	public boolean generate(World world, Random rand, int x, int y, int z){
		...
		for(int yy=0; yy<baseHeight; yy++)world.setBlock(x,y+yy,z,Block.wood.blockID,3,3);

		y+=baseHeight;
		for(int branch=0; branch<branchAmount; branch++){
			// wood in the center
			for(int a=0; a<3; a++)world.setBlock(x,y+branch*3+a,z,Block.wood.blockID,3,3);
			// + shaped leaves
			for(int a=0; a<2; a++){
				for(int b=0; b<4; b++){
			...
			for(int a=0; a<4; a++)world.setBlock(x+offsetX[a]*2,y+branch*3+1,z+offsetZ[a]*2,Block.leaves.blockID,3,3);
		}
