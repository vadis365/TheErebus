...
public final class OreSettings{
	private static final byte[] checkX = new byte[]{ -1, -1, 1, 1, 0, 0 },
				...
				for(a = 0; a < 6; a++){
					...
					if (world.isAirBlock(testX,testY,testZ)){
						if ((oreAmount = minAmount+rand.nextInt(maxAmount-minAmount+1)) == 1){
							if (world.getBlockId(xx,yy,zz) == ModBlocks.umberstone.blockID)world.setBlock(xx,yy,zz,oreType.oreId,oreType.oreMeta,2);
						}
						else{
