...
public class EntityCicada extends EntityCreature {
			...
			for (int q1 = p; q1 < q; q1++)
				for (int n2 = n1; n2 < o1; n2++) {
					...
					if (o2 == ModBlocks.logErebusGroup3.blockID)
						// if(worldObj.isAirBlock(p1, q1+1, n2) &&
						// worldObj.isAirBlock(p1, q1-1, n2))
						return true;
				}
		return false;
	}
	...
	protected Entity findEnemyToAttack() {
		List list = worldObj.getEntitiesWithinAABB(
				...
				AxisAlignedBB.getBoundingBox(posX + 0.5D, posY + 0.5D,
						posZ + 0.5D, posX + 0.5D, posY + 0.5D, posZ + 0.5D)
						.expand(sonics * 0.2D, 0.5D, sonics * 0.2D));
