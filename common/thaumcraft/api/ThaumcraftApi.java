 ...
 * 
 * 
 *         IMPORTANT: If you are adding your own aspects to items it is a good
 *         idea to do it AFTER Thaumcraft adds its aspects, otherwise odd things
 *         may happen.
 * 
 */
public class ThaumcraftApi {
	...
	// Miscellaneous
	/**
	 * Portable Hole Block-id Blacklist. Simply add the block-id's of blocks you
	 * don't want the portable hole to go through.
	 */
	public static ArrayList<Integer> portableHoleBlackList = new ArrayList<Integer>();

	// RESEARCH/////////////////////////////////////////
	// public static Document researchDoc = null;
	// public static ArrayList<String> apiResearchFiles = new
	// ArrayList<String>();
	...
	/**
	 * This is used to add aspects to entities which you can then scan using a
	 * thaumometer. Also used to calculate vis drops from mobs.
	 * 
	 * @param entityName
	 * @param aspects
	 * @param nbt
	 *            you can specify certain nbt keys and their values to
	 *            differentiate between mobs. <br>
	 *            For example the normal and wither skeleton: <br>
	 *            ThaumcraftApi.registerEntityTag("Skeleton", (new
	 *            AspectList()).add(Aspect.DEATH, 5)); <br>
	 *            ThaumcraftApi.registerEntityTag("Skeleton", (new
	 *            AspectList()).add(Aspect.DEATH, 8), new
	...
	/**
	 * This method is used to determine what bonus items are generated when the
	 * infernal furnace smelts items
	 * 
	 * @param in
	 *            The result (not input) of the smelting operation. e.g. new
	 *            ItemStack(ingotGold)
	 * @param out
	 *            The bonus item that can be produced from the smelting
	 *            operation e.g. new ItemStack(nuggetGold,0,0). Stacksize should
	 *            be 0 unless you want to guarantee that at least 1 item is
	 *            always produced.
	 */
	public static void addSmeltingBonus(ItemStack in, ItemStack out) {
		smeltingBonus.put(Arrays.asList(in.itemID, in.getItemDamage()), new ItemStack(out.itemID, 0, out.getItemDamage()));
	}

	/**
	 * Returns the bonus item produced from a smelting operation in the infernal
	 * furnace
	 * 
	 * @param in
	 *            The result of the smelting operation. e.g. new
	 *            ItemStack(ingotGold)
	...
	/**
	 * Excludes specific items from producing bonus items when they are smelted
	 * in the infernal furnace, even if their smelt result would normally
	 * produce a bonus item.
	 * 
	 * @param in
	 *            The item to be smelted that should never produce a bonus item
	 *            (e.g. the various macerated dusts form IC2) Even though they
	 *            produce gold, iron, etc. ingots, they should NOT produce bonus
	 *            nuggets as well.
	 */
	...
	/**
	 * Sees if an item is allowed to produce bonus items when smelted in the
	 * infernal furnace
	 * 
	 * @param in
	 *            The item you wish to check
	 ...
	 *            if it will work without research
	 * @param result
	 *            the recipe output. It can either be an itemstack or an nbt
	 *            compound tag that will be added to the central item
	 * @param instability
	 *            a number that represents the N in 1000 chance for the infusion
	 *            altar to spawn an instability effect each second while the
	 *            crafting is in progress
	 * @param aspects
	 *            the essentia cost per aspect.
	 * @param aspects
	 *            input the central item to be infused
	 * @param recipe
	 *            An array of items required to craft this. Input itemstacks are
	 *            NBT sensitive. Infusion crafting components are automatically
	 *            "fuzzy" and the oredict will be checked for possible matches.
	 * 
	 */
	...
	/**
	 * @param key
	 *            the research key required for this recipe to work.
	 * @param result
	 *            the output result
	 * @param cost
	 *            the vis cost
	 * @param tags
	 *            the aspects required to craft this
	 */
	...
	/**
	 * Checks to see if the passed item/block already has aspects associated
	 * with it.
	 * 
	 * @param id
	 * @param meta
	 ...
	 * declaration for cobblestone:
	 * <p>
	 * <i>ThaumcraftApi.registerObjectTag(Block.cobblestone.blockID, new
	 * int[]{0,1}, (new ObjectTags()).add(EnumTag.ROCK,
	 * 1).add(EnumTag.DESTRUCTION, 1));</i>
	 * 
	 * @param id
	 * @param meta
	 *            A range of meta values if you wish to lump several item meta's
	 *            together as being the "same" item (i.e. stair orientations)
	 * @param aspects
	 *            A ObjectTags object of the associated aspects
	 */
	public static void registerObjectTag(int id, int[] meta, AspectList aspects) {
	...
	/**
	 * Used to assign aspects to the given item/block. Attempts to automatically
	 * generate aspect tags by checking registered recipes. Here is an example
	 * of the declaration for pistons:
	 * <p>
	 * <i>ThaumcraftApi.registerComplexObjectTag(Block.pistonBase.blockID, 0,
	 * (new ObjectTags()).add(EnumTag.MECHANISM, 2).add(EnumTag.MOTION, 4));</i>
	 * 
	 * @param id
	 * @param meta
	 *            pass -1 if all damage values of this item/block should have
	...
	/**
	 * To define mod crops you need to use FMLInterModComms in your @Mod.Init
	 * method. There are two 'types' of crops you can add. Standard crops and
	 * clickable crops.
	 * 
	 * Standard crops work like normal vanilla crops - they grow until a certain
	 * metadata value is reached and you harvest them by destroying the block
	 * and collecting the blocks. You need to create and ItemStack that tells
	 * the golem what block id and metadata represents the crop when fully
	 * grown. Example for vanilla wheat:
	 * FMLInterModComms.sendMessage("Thaumcraft", "harvestStandardCrop", new
	 * ItemStack(Block.crops,1,7));
	 * 
	 * Clickable crops are crops that you right click to gather their bounty
	 * instead of destroying them. As for standard crops, you need to create and
	 * ItemStack that tells the golem what block id and metadata represents the
	 * crop when fully grown. The golem will trigger the blocks onBlockActivated
	 * method. Example (this will technically do nothing since clicking wheat
	 * does nothing, but you get the idea):
	 * FMLInterModComms.sendMessage("Thaumcraft", "harvestClickableCrop", new
	 * ItemStack(Block.crops,1,7));
	 */
